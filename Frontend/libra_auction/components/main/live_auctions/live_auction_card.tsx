import type { LiveAuction } from "@/types/auction/live_auction";
    ...
    <div className="flex flex-col h-full rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="h-full bg-white p-6 flex flex-col gap-4">
        ...
        <div className="flex pt-1">
          ...
          <div className="flex-1"></div>
          <a className="mt-auto bg-(--primary-color) text-white px-6 py-2 font-bold hover:bg-(--primary-color)/90 active:bg-(--primary-color)/80" href={`auctions/${card.category_id}/${card.id}`}>
            Place Bid
          </a>
        </div>
      </div>
