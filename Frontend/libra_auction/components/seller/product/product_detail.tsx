...
export const ProductDetail = ({ data }: { data: ProductDetailData }) => {
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 bg-white p-6 md:p-10 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <div className="flex flex-col">
                <nav className="text-xs font-bold text-[var(--primary-color)] uppercase tracking-widest mb-2">
                    {data.category_name}
                </nav>
                ...
                <div className="flex items-center gap-6 mb-6 pb-6 border-b border-gray-50">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-mono mt-1">#{data.id}</p>
                    </div>
                </div>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {data.attributes.map((attr, idx) => (
                            <div key={idx} className="flex justify-between p-3 bg-[var(--background-color)] rounded-xl border border-gray-50">
                ...
                <div className="flex gap-3 mt-auto pt-6">
                    <Link
                        href={`/seller-dashboard/products/${data.id}/edit`}
                        className="flex-1 flex justify-center items-center gap-2 bg-[var(--primary-color)] hover:bg-[var(--secondary-color)] text-white font-bold py-3 rounded-xl transition-all active:scale-95"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /><path d="m15 5 4 4" /></svg>
                    Sửa tài sản
                </Link>
                <Link
                    href={`/seller-dashboard/products/${data.id}/delete`}
